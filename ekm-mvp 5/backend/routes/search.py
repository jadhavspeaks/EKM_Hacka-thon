...
async def search(
    ...
    # Pre-group by source so frontend renders with zero filtering
    confluence_results = [r for r in results if r.get("source_type") == "confluence"]
    other_results      = [r for r in results if r.get("source_type") != "confluence"]
    ...
    return {
        ...
        "has_confluence":      len(confluence_results) > 0,
        "page":                page,
        "page_size":           page_size,
        "smes":                smes,
