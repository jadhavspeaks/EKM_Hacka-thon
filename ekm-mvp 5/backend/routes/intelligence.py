...
def clear_cache(source: str | None = None):
    """
    Bust the intelligence cache.
    Called after every successful sync so new data is visible immediately.
    Pass source to clear only keys related to that source, or None to clear all.
    """
    if source is None:
        _CACHE.clear()
        logger.info("Intelligence cache cleared (full)")
    else:
