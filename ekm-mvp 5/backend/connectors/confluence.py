...
async def list_spaces() -> list[dict]:
    """Return all accessible Confluence spaces as [{key, name, type}]."""
    client = _get_client()
    if not client:
        return []
    try:
        result = client.get_all_spaces(limit=200)
        return [
            {"key": s["key"], "name": s.get("name", s["key"]), "type": s.get("type", "global")}
            for s in result.get("results", [])
        ]
    except Exception as e:
        logger.error(f"Confluence list_spaces failed: {e}")
        return []


async def fetch_documents(updated_since: datetime | None = None,
                          spaces_override: list[str] | None = None) -> list[Document]:
    client = _get_client()
    if not client:
        return []

    spaces = spaces_override if spaces_override else settings.confluence_space_list
    if not spaces:
        try:
