          ...
          return (
            <div key={sme.name} className="px-3 py-3 hover:bg-slate-50 transition-colors">
              <div className="flex items-center gap-1.5 mb-1">
                <span className="text-sm shrink-0">{i < 3 ? medals[i] : <span className="text-xs text-slate-400">{i+1}</span>}</span>
              ...
              <div className="flex items-center gap-1.5 mb-1.5">
                ...
                <span className="text-xs text-slate-400">{sme.doc_count} docs</span>
              </div>
          ...
          <div className="flex items-start justify-between gap-2 mb-1">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <span className="text-xs text-slate-400">{doc.source}</span>
                {doc.author && (
          ...
          {doc.tags?.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {doc.tags.slice(0,5).map(t => (
                <span key={t} className="text-xs bg-slate-50 text-slate-500 border border-slate-200 px-1.5 py-0.5 rounded-full">
                  {t}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                {m.author_name && <span className="text-xs text-slate-400">{m.author_name}</span>}
                {doc.source && <span className="text-xs text-slate-400">· {doc.source}</span>}
              </div>
            </div>
            {doc.url && (
              <a href={doc.url} target="_blank" rel="noreferrer"
    ...
    <div className="min-h-full bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-5">
        <div className="max-w-5xl mx-auto">
          <h1 className="text-lg font-bold text-slate-900 mb-1">Search</h1>
          <p className="text-xs text-slate-400 mb-4">BM25 · entity extraction · SME ranking · Confluence-first</p>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-teal-400 focus:bg-white transition-all"
                />
                {inputVal && (
