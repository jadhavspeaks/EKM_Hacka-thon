...
async def list_projects() -> list[dict]:
    """Return all accessible Jira projects as [{key, name, type}]."""
    client = _get_client()
    if not client:
        return []
    try:
        ...
        return [
            {"key": p["key"], "name": p.get("name", p["key"]), "type": p.get("projectTypeKey", "software")}
            for p in (all_projects or [])
        ]
    except Exception as e:
        logger.error(f"Jira list_projects failed: {e}")
        return []


async def fetch_documents(updated_since: datetime | None = None,
                          projects_override: list[str] | None = None) -> list[Document]:
    client = _get_client()
    if not client:
        return []

    projects = projects_override if projects_override else settings.jira_project_list
    if not projects:
        try:
            all_projects = client.projects()
