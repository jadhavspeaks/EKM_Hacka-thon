...
async def debug_velocity():
    ...
    with_date = await db.documents.count_documents({"updated_at": {"$exists": True, "$ne": None}})
...
async def get_velocity(topic: str = Query(None)):
    ...
    def parse_date(val):
        ...
        if isinstance(val, str):
            ...
            for fmt in [
                ...
                try:
                    dt = datetime.strptime(val.replace("Z", "+00:00").replace("+00:00", ""), fmt.rstrip("Z").rstrip("%z"))
    ...
    total  = sum(sum(s[src] for src in ["confluence","jira","github","sharepoint"]) for s in series)
