...
Intelligence Routes
────────────────────
GET /api/intelligence/health GET /api/intelligence/risk GET /api/intelligence/onboarding """
-- content freshness, knowledge audit, untouched docs
-- vendor dependency + concentration risk per topic
-- top 10 docs for a topic (onboarding path)
import re
import logging
import time
...
source_stats[st] = {"fresh_30": 0, "fresh_90": 0, "fresh_365": 0, "total": source_stats[st]["total"] += 1
0}
...
vendor_pct = round(vendor_contribs / total_contribs * 100) if total_contribs internal_pct = round(internal_contribs / total_contribs * 100) if total_contribs else
...
Sources are grouped and ordered: Confluence -> SharePoint -> GitHub -> Jira (reference on
"""
...
"""
db = get_db()
Undocumented systems: topics active in Jira/GitHub but with zero Confluence/SharePoint pa
cursor = db.documents.find(
{},
...
if is_inactive or (is_vendor and topic_count >= 3) else
)
at_risk.append({
"name": person,
"type": ps["type"],
...
"recommendation": (
"Urgent knowledge transfer -- vendor + inactive." if (is_inactive and is_vendo
f"Schedule knowledge transfer -- inactive {days_inactive}d." if is_inactive el
...
grade = "A" if score >= 80 else "B" if score >= 60 else "C" if score >= 40 else "D" i
missing = [st for st in ["confluence", "sharepoint", "github"] if sources.get(st, 0)
coverage.append({
"topic": tag,
"score": score,
"grade": grade,
...
"recommendation": (
f"Add Confluence docs for '{tag}'." if "confluence" in missing and sources.ge
...
return _cache_set("coverage", result)
@router.get("/velocity/debug")
...
db = get_db()
now = datetime.now(timezone.utc)
# Sample 20 docs and inspect their date fields
sample = await db.documents.find(
{}, {"source_type": 1, "updated_at": 1, "ingested_at": 1, "title": 1}
).limit(20).to_list(length=20)
date_info = []
for doc in sample:
ud = doc.get("updated_at")
ia = doc.get("ingested_at")
date_info.append({
"source_type": doc.get("source_type"),
"title_snippet": str(doc.get("title", ""))[:40],
"updated_at": str(ud),
"updated_at_type": type(ud).__name__,
"ingested_at": str(ia),
"ingested_at_type": type(ia).__name__,
})
# Count docs with each date type
total = await db.documents.count_documents({})
with_date = await db.documents.count_documents({"updated_at": {"$exists": True, "$ne": No
with_bson = await db.documents.count_documents({"updated_at": {"$type": "date"}})
...
return {
"total_docs": total,
"with_updated_at": with_date,
"updated_at_bson_date": with_bson,
"updated_at_string": with_str,
"bson_dates_in_last_12mo": bson_recent,
"note": "If updated_at_string > 0 and bson_dates_in_last_12mo == 0, velocity uses str
"sample": date_info,
}
@router.get("/velocity")
...
b[label] = {"confluence": 0, "jira": 0, "github": 0, "sharepoint": 0, "label": la
return b
...
cursor2 = db.documents.find(all_filt, {"source_type": 1, "updated_at": 1, "ingested_at":
...
try:
dt = datetime.strptime(val.replace("Z", "+00:00").replace("+00:00", ""),
return dt.replace(tzinfo=timezone.utc)
except:
pass
try:
return datetime.fromisoformat(val.replace("Z", "+00:00")).replace(tzinfo=time
except:
pass
return None
# Populate buckets from all docs with parsed dates
parsed_count = 0
date_range = []
for doc in all_docs:
...
total = sum(sum(s[src] for src in ["confluence","jira","github","sharepoint"]) for s in
# ── Step 4: If still nothing, expand window to all-time ──────────────────
if total == 0 and date_range:
# Rebuild around actual date range
earliest = min(date_range)
latest = max(date_range)
# Build monthly buckets from earliest to now
expanded: dict[str, dict] = {}
cursor_dt = datetime(earliest.year, earliest.month, 1, tzinfo=timezone.utc)
while cursor_dt <= now:
label = cursor_dt.strftime("%b %Y")
expanded[label] = {"confluence":0,"jira":0,"github":0,"sharepoint":0,"label":labe
...
st = doc.get("source_type", "")
if st in expanded[label]:
expanded[label][st] += 1
series = list(expanded.values())
total = sum(sum(s[src] for src in ["confluence","jira","github","sharepoint"]) for s
# ── Trend ─────────────────────────────────────────────────────────────────
recent = sum(sum(s[src] for src in ["confluence","jira","github","sharepoint"]) for s i
previous = sum(sum(s[src] for src in ["confluence","jira","github","sharepoint"]) for s i
...
dt = now - relativedelta(months=i)
label = dt.strftime("%b %Y")
buckets[label] = {"confluence": 0, "jira": 0, "github": 0, "sharepoint": 0, "label":
since = now - relativedelta(months=12)
# Broad filter -- any date field
date_filter: list[dict] = [
{"updated_at": {"$gte": since}},
{"ingested_at": {"$gte": since}},
]
filt: dict = {"$or": date_filter}
if topic:
filt["tags"] = {"$regex": re.escape(topic), "$options": "i"}
cursor = db.documents.find(
filt, {"source_type": 1, "updated_at": 1, "ingested_at": 1}
).limit(100000)
docs = await cursor.to_list(length=100000)
# If nothing in window, fall back to all docs (show what we have)
if not docs:
fallback_filt: dict = {}
if topic:
fallback_filt["tags"] = {"$regex": re.escape(topic), "$options": "i"}
cursor2 = db.documents.find(
fallback_filt, {"source_type": 1, "updated_at": 1, "ingested_at": 1}
).limit(10000)
docs = await cursor2.to_list(length=10000)
# Rebuild buckets around actual data range
if docs:
dates = []
for d in docs:
dt2 = d.get("updated_at") or d.get("ingested_at")
if isinstance(dt2, str):
try: dt2 = datetime.fromisoformat(dt2.replace("Z", "+00:00"))
except: continue
if dt2:
dates.append(dt2 if dt2.tzinfo else dt2.replace(tzinfo=timezone.utc))
if dates:
earliest = min(dates)
buckets = {}
for i in range(11, -1, -1):
dt3 = now - relativedelta(months=i)
label = dt3.strftime("%b %Y")
buckets[label] = {"confluence":0,"jira":0,"github":0,"sharepoint":0,"labe
# Populate buckets
for doc in docs:
dt4 = doc.get("updated_at") or doc.get("ingested_at")
if not dt4:
continue
if isinstance(dt4, str):
try: dt4 = datetime.fromisoformat(dt4.replace("Z", "+00:00"))
except: continue
if dt4.tzinfo is None:
dt4 = dt4.replace(tzinfo=timezone.utc)
label = dt4.strftime("%b %Y")
if label in buckets:
st = doc.get("source_type", "")
...
Returns all systems they own, open tickets, key docs, contacts -- structured as a checklis
"""
db = get_db()
